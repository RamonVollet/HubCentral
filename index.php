...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $p = $_POST['password'] ?? '';
    if ($u === '' || $p === '') { $errors[] = 'Preencha usuário e senha.'; }
    else { $user = find_user($u);
        if (!$user || !password_verify($p, $user['password'])) { $errors[] = 'Usuário ou senha inválidos.'; }
        else { $_SESSION['user'] = ['username' => $user['username'], 'role' => $user['role'], 'access' => $user['access'] ]; set_user_session_id($user['username'], session_id()); header('Location: php/dashboard.php'); exit; } }
}
?>
<!doctype html>
<html lang="pt-BR">
...
<body>
  <!-- Banner Superior -->
  <div class="banner">Login - HubCentral</div>
  ...
  <div class="content-wrapper">
    <div class="login-container">
      ...
      <div class="welcome">Bem-vindo ao HubCentral!</div>
      ...
      <form method="post" action="">
        ...
        <a class="register-link" href="php/register.php">Criar conta</a>
      </form>
    </div>
  </div>
